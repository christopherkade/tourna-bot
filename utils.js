...
export function getActiveTournamentsArray() {
  const tournamentKeys = Object.keys(activeTournaments);

  return tournamentKeys.map((key) => {
    const tournamentData = activeTournaments[key];

    return {
      label: tournamentData.title,
      value: key,
    };
  });
}
